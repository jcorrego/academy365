...
@section('content')
    <x-pages.header title="Edit question for: {{ $test->course->name }}"
                    description="Here you can update this question and its options"></x-pages.header>
    <div class="mx-auto sm:px-6 lg:px-8 my-8">
        <form action="{{ route('question-update',[$test->id, $question->id]) }}" method="POST">
            @csrf
            @method('put')
                ...
                <div class="mt-5 md:mt-0 md:col-span-2">
                    <div class="shadow sm:rounded-md sm:overflow-hidden">
                        <div class="px-4 py-5 bg-white sm:p-6 space-y-6">
                            <x-forms.input id="name" label="Question" placeholder="Ex. What was the main topic?" value="{{ $question->name }}"></x-forms.input>
                            <x-forms.textarea id="description" label="Description" placeholder="Describe the question in detail" value="{{ $question->description }}" help="Question description text to be presented to the user."></x-forms.textarea>
                        </div>
                    </div>
                </div>
            </div>
